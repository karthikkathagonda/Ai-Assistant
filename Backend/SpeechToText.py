...
chrome_options.add_argument("--headless")   #Replaced --headless=new with --headless
...
# Function to write the assistant's status by writing it to a file
def SetAssistantStatus(Status):
    with open(rf"{TempDirPath}/Status.data", "w", encoding="utf-8") as file:
        file.write(Status)

# Function to modify a query for interpretation and formatting
def QueryModifier(Query):
    new_query = Query.lower().strip()
    query_words = new_query.split()
    question_words = ["how", "what", "who", "where", "when", "why", "which", "whose", "whom", "can you", "what's", "which"]
...
def SpeechRecognition():
    ...
    while True:
        try:
            ...
            print(f"Error during speech recognition: {e}")  # Log the error
